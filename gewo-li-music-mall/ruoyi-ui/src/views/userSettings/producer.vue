<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="审核状态" align="center" prop="producerReviewStatus">
        <template slot-scope="scope">
            <el-tag type="info" v-if="scope.row.producerReviewStatus=='1'">未审核</el-tag>
            
            <el-popover
             v-else-if="scope.row.producerReviewStatus=='2'"
              ...
              <div>
                  <el-button type="warning" size="mini" @click="doProducerReviewStatus(scope.row,4)">审核不通过</el-button>
                  <el-button type="success" size="mini" @click="doProducerReviewStatus(scope.row,3)">审核通过</el-button>
              </div>
              
              <el-button type="primary" size="mini" slot="reference">待审核</el-button>
            </el-popover>
            
            <el-tag type="success" v-else-if="scope.row.producerReviewStatus=='3'">审核通过</el-tag>
            <el-tag type="warning" v-else-if="scope.row.producerReviewStatus=='4'">审核不通过</el-tag>
        </template>
      </el-table-column>
...
<script>
import { getProducerUser, getUser, delUser, addUser, updateUser, producerReview } from "@/api/system/userSettings";
...
export default {
  ...
  methods: {
      doProducerReviewStatus(item,status){
          let postData = {
              userId:item.id,
              status
          }
          
          producerReview(postData).then(res => {
              item.showView = true;
              this.getList()
          })
      },
    ...
    getList() {
      this.loading = true;
      getProducerUser(this.queryParams).then(response => {
