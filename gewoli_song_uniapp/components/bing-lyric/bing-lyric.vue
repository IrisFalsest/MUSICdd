<template>
	<view class="lyric-main" :style="{
		...
		<view class="center-view" :style="{top:centerLineTop,display:showCenterView?'flex':'none',fontSize:cuCenterStyle.fontSize,height:cuCenterStyle.height}">
			...
			<view class="center-line" :style="{height:cuCenterStyle.lineHeight}"></view>
			<view class="center-time">{{centerTime}}</view>
		</view>
        
        <!-- 全选 复制 取消 -->
		<view class="selectControl" v-if="selectStatus" :style="{color:cuSelectControlStyle.color,fontSize:cuSelectControlStyle.itemFontSize}">
			<view class="selectAll" @click="selectAll" :style="{borderRadius:cuSelectControlStyle.itemBorderRadius}">{{selectAllStatus ? '全不选' : '全选'}}</view>
			<view class="copy" @click="copyLyric" :style="{borderRadius:cuSelectControlStyle.itemBorderRadius,}">复制歌词</view>
			<view class="cancel" @click="selectCancel" :style="{borderRadius:cuSelectControlStyle.itemBorderRadius}">取消</view>
		</view>
	...
	export default{
		data() {
			return {
				...
				curLyricIndex: 0,
				selectStatus: false,  //按钮组状态
				touchStatus: false,
				showCenterView: false, //歌词视图
				spaceLineNum: 0,
				timeout: {
					centerViewHide: null,
					goCenter: null,
